...
vec3_t	ofxMd2::anorms[ NUMVERTEXNORMALS ] = {
#include	"anorms.h"
};

// precalculated dot product results
float	ofxMd2::anorms_dots[ SHADEDOT_QUANT ][256] = {
#include	"anormtab.h"
};

static float	*shadedots = ofxMd2::anorms_dots[0];
...
void ofxMd2::loadModel(string _MD2file, string _TGAfile, float _size, float _fps){
	
	if ( loadModel(ofToDataPath(_MD2file).c_str()) ){ 
		if ( loadSkin(ofToDataPath(_TGAfile).c_str()) ){
			cout << "MODEL AND TEXTURE LOADED" << endl;
			animlist[0].last_frame	= num_frames;
			animlist[0].fps	= _fps;
			setAnim( 0 );
			scaleModel( _size );
		} else cout << "MODEL LOADED BUT DON´T TEXTURE" << endl;
	} else cout << "FAIL LOADING MODEL" << endl;
}
...
bool ofxMd2::loadModel( const char *filename ){
...
/*
 Formatos BMP y TGA (con o sin compresión RLE).
 
 En TGA si la direcion de lectura es abajo-arriba
 en la funcion LoadFileTGA( filename, &texels, &width, &height, true );
 de texture.cpp poner el ultimo true a false.
 
 */

bool ofxMd2::loadSkin( const char *filename ){
...
void ofxMd2::drawModel( float time ){
	
	// animate. calculate current frame and next frame
	if( time > 0.0 ) animate( time );
	...
	for(int i = 0; i < rotations.size(); i++){
		glRotatef(rotations[i].angle, rotations[i].x, rotations[i].y, rotations[i].z);
	}
...
void ofxMd2::animate( float time ){
	...
	if( m_anim.curr_time - m_anim.old_time > (1.0 / m_anim.fps) ){
		...
		if( m_anim.next_frame > m_anim.endframe ) m_anim.next_frame = m_anim.startframe;
		m_anim.old_time = m_anim.curr_time;
	}
	
	// prevent having a current/next frame greater
	// than the total number of frames...
	if( m_anim.curr_frame > (num_frames - 1) )	m_anim.curr_frame = 0;
	if( m_anim.next_frame > (num_frames - 1) )	m_anim.next_frame = 0;
...
void ofxMd2::processLighting( void ){
...
void ofxMd2::interpolate( vec3_t *vertlist ){
...
void ofxMd2::drawFrame( int frame ){
...
void ofxMd2::setRotation(float _angle, float _x, float _y, float _z){
	glRot r = glRot();
