...
int LoadFileBMP( const char *filename, unsigned char **pixels, int *width, int *height, bool flipvert )
{
	...
	BITMAPINFOHEADER_MD2	*bmih;			// bitmap info header (windows)
	BITMAPCOREHEADER_MD2	*bmch;			// bitmap core header (os/2)
	RGBTRIPLE_MD2		*os2_palette;	// pointer to the color palette os/2
	RGBQUAD_MD2			*win_palette;	// pointer to the color palette windows
	...
	bmch = (BITMAPCOREHEADER_MD2 *)pBuff;
	bmih = (BITMAPINFOHEADER_MD2 *)pBuff;


	if( (bmih->biCompression < 0) || (bmih->biCompression > 3) )
	{
		// OS/2 style
		pBuff += sizeof( BITMAPCOREHEADER_MD2 );
	...
	{
		// WINDOWS style
		pBuff += sizeof( BITMAPINFOHEADER_MD2 );
	...
	if( bitCount <= 8 )
	{
		...
		os2_palette = (RGBTRIPLE_MD2 *)pBuff;
		win_palette = (RGBQUAD_MD2 *)pBuff;
...
// RGB 24 BITS
						for( col = 0; col < w; col++, ptr += 4 )
						{
							...
							RGBTRIPLE_MD2 *pix = (RGBTRIPLE_MD2 *)pBuff;
							pBuff += sizeof( RGBTRIPLE_MD2 );
...
// RGB 32 BITS
						for( col = 0; col < w; col++, ptr += 4 )
						{
							...
							RGBQUAD_MD2 *pix = (RGBQUAD_MD2 *)pBuff;
							pBuff += sizeof( RGBQUAD_MD2 );
